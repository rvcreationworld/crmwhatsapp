...
const DashboardLayout = () => {
  ...
  const adminControlLinks = [
    ...
    { name: 'BOT Auto Assign', path: '/admin/bot-auto-assign', icon: <Target size={20} /> },
  ...
  const whatsappControlLinks = [
    { name: 'Overview', path: '/admin/whatsapp-center?tab=overview', icon: <BarChart3 size={20} /> },
    { name: 'Conversations', path: '/admin/whatsapp-center?tab=conversations', icon: <MessageCircle size={20} /> },
    { name: 'Logs', path: '/admin/whatsapp-center?tab=logs', icon: <ListTodo size={20} /> },
    { name: 'Queue', path: '/admin/whatsapp-center?tab=queue', icon: <ListTodo size={20} /> },
    { name: 'Inbound', path: '/admin/whatsapp-center?tab=inbound', icon: <Inbox size={20} /> },
    { name: 'Service Messages', path: '/admin/whatsapp-center?tab=service-messages', icon: <Settings size={20} /> },
    { name: 'Automation Messages', path: '/admin/whatsapp-center?tab=automations', icon: <Settings size={20} /> },
    { name: 'Recurring Broadcasts', path: '/admin/whatsapp-center?tab=recurring-broadcasts', icon: <Repeat size={20} /> },
    { name: 'Templates', path: '/admin/whatsapp-center?tab=templates', icon: <LayoutTemplate size={20} /> },
    { name: 'Media Library', path: '/admin/whatsapp-center?tab=media-library', icon: <ImageIcon size={20} /> },
    { name: 'Interakt Agents', path: '/admin/whatsapp-center?tab=interakt-agents', icon: <Users size={20} /> },
  ...
  const renderLink = (link, isSubmenu = false) => {
    // Exact match for most paths, but query string match for whatsapp-center tabs
              ...
              <>
                ...
                <li className="pt-2 mt-2 border-t border-white/5">
                  <button
                    onClick={() => {
                      if (sidebarCollapsed) setSidebarCollapsed(false);
                      setWhatsappControlOpen(!whatsappControlOpen);
                    }}
                    className={cn(
                      "flex items-center gap-3 w-full rounded-lg transition-all duration-200 group relative text-left",
                      sidebarCollapsed ? "justify-center p-3" : "px-3 py-2.5",
                    ...
                    title={sidebarCollapsed ? "WhatsApp Control" : undefined}
                  >
                    <div className="transition-transform duration-200 group-hover:scale-110 shrink-0 text-emerald-500/80 group-hover:text-emerald-400">
                      <MessageCircle size={20} />
                    </div>
                    {!sidebarCollapsed && (
                      <>
                        <span className="truncate flex-1 font-semibold text-sm tracking-wide text-slate-300">WhatsApp Control</span>
                        <ChevronDown size={16} className={cn("transition-transform duration-200", whatsappControlOpen ? "rotate-180" : "")} />
                      </>
                    )}
                  </button>
                </li>
