...
const UntouchedBotLeads = () => {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get('/api/telecaller/untouched-leads?type=bot');
      return res.data;
    }
  });
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Bot leads that have not been called yet.</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white dark:bg-[#1e1e2f] border border-slate-200 dark:border-slate-800 rounded-2xl p-12 text-center shadow-sm">
          ...
          <p className="text-slate-500 max-w-md mx-auto">
            No untouched bot leads. All assigned bot leads have been called. Wait for new leads to be assigned.
          </p>
        </div>
      ) : viewMode === 'grid' ? (
...
export default UntouchedBotLeads;
