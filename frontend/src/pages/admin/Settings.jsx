...
import { RefreshCw, Phone, Shield, X, ChevronRight, Zap, Users } from "lucide-react";
...
const Settings = () => {
  ...
  useEffect(() => {
    if (settingsData) {
      ...
      if (settingsData.bot_auto_assignment_top_count !== undefined) setBotAutoAssignTopCount(settingsData.bot_auto_assignment_top_count);
    }
  }, [settingsData]);
  ...
  const handleBotAutoAssignSubmit = (e) => {
    e.preventDefault();
    updateSettingsMutation.mutate({ bot_auto_assignment_top_count: botAutoAssignTopCount });
  };
  ...
  if (activeModal) {
    ...
    if (activeModal === 'bot-auto-assign') {
      title = "BOT Auto Assignment";
      icon = Users;
      content = (
        <form onSubmit={handleBotAutoAssignSubmit} className="space-y-6">
          <div>
            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Top Telecallers Count</label>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">
              Number of top telecallers to consider for BOT auto-assignment. (Min 1, Max 500)
            </p>
            <input 
              type="number" 
              min="1"
              max="500"
              required
              value={botAutoAssignTopCount}
              onChange={e => setBotAutoAssignTopCount(e.target.value)}
          ...
          <div className="pt-2">
            ...
            >
              {updateSettingsMutation.isLoading ? 'Saving...' : 'Save Setting'}
            </button>
          </div>
        </form>
      );
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 pb-10">
      ...
      <div className="grid gap-4">
        ...
        <SettingBlock 
          icon={Users}
          title="BOT Auto Assignment"
          description="Configure how many top telecallers receive BOT leads automatically."
          value={settingsLoading ? "..." : `Top ${botAutoAssignTopCount}`}
          isLoading={settingsLoading}
          onClick={() => setActiveModal('bot-auto-assign')}
        />
