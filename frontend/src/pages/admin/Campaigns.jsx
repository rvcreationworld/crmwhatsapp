...
const Campaigns = () => {
  ...
  const toggleAutoSyncMutation = useMutation({
    mutationFn: async ({ id, auto_sync }) => {
      const res = await api.patch(`/api/campaigns/${id}/auto-sync`, { auto_sync: auto_sync ? 1 : 0 });
      return res.data;
    },
    onSuccess: () => {
    ...
    onError: (error) => {
      toast.error(error.response?.data?.message || "Failed to update auto-sync");
    }
  });

  const handleToggleAutoSync = (id, auto_sync) => {
    toggleAutoSyncMutation.mutate({ id, auto_sync });
  };
            ...
            <table className="w-full text-left border-collapse whitespace-nowrap">
              <thead className="sticky top-0 z-10 bg-slate-50/90 dark:bg-[#151521]/90 backdrop-blur-md shadow-sm">
                <tr className="text-slate-600 dark:text-slate-300 text-sm font-semibold">
                  ...
                  <th className="p-4 border-b border-slate-200 dark:border-slate-700">Auto Sync</th>
                      ...
                      <div className="flex items-center gap-2">
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            ...
                            checked={campaign.auto_sync === 1}
                            onChange={(e) => handleToggleAutoSync(campaign.id, e.target.checked)}
                          />
                          <div className="w-9 h-5 bg-slate-200 dark:bg-slate-700 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-purple-600"></div>
                        </label>
                        <span className={`text-xs font-medium ${campaign.auto_sync === 1 ? 'text-purple-600 dark:text-purple-400' : 'text-slate-500'}`}>
                          {campaign.auto_sync === 1 ? 'On' : 'Off'}
                        </span>
                      </div>
                    </td>
                    <td className="p-4">
