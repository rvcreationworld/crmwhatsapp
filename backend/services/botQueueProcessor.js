const db = require('../config/db');
const { sendSessionMessage } = require('./hubService');
const { assignChatToAgent } = require('./interaktAssignmentService');
const { syncLeadTraits } = require('./interaktTraitSyncService');
const { renderServiceMessage } = require('./whatsappServiceMessageService');
const { queueAutomations } = require('./automationQueueService');
let autoAssignService = null; // Lazy load to avoid circular dependency

const sendBotAssignmentMessage = async (workingSheetId, telecallerId) => {
    try {
        const [convos] = await db.query(
            `SELECT * FROM whatsapp_conversations WHERE lead_table = 'working_sheet' AND lead_id = ? LIMIT 1`,
            [workingSheetId]
        );
        
        if (convos.length === 0) return;
        const convo = convos[0];
        
        if (convo.customer_response !== 'INTERESTED') return;
        
        if (convo.service_window_expires_at && new Date(convo.service_window_expires_at) < new Date()) {
            await db.query(`UPDATE whatsapp_conversations SET rm_session_message_status = 'WINDOW_EXPIRED' WHERE lead_table = 'working_sheet' AND lead_id = ?`, [workingSheetId]);
            return;
        }
        
        if (convo.rm_session_message_status === 'SENT') return;
        
        const [rms] = await db.query(`SELECT telecaller_name, tele_mobile, phone_last10 FROM telecaller_master WHERE id = ? LIMIT 1`, [telecallerId]);
        let rmFound = false;
        let rmMobile = null;
        let rmName = null;

        if (rms.length > 0) {
            rmName = rms[0].telecaller_name || 'RM';
            rmMobile = rms[0].tele_mobile;
            if (!rmMobile || rmMobile.trim() === '') rmMobile = rms[0].phone_last10;
            if (rmMobile && rmMobile.trim() !== '') rmFound = true;
        }

        if (!rmFound) {
            await db.query(`UPDATE whatsapp_conversations SET rm_session_message_status = 'RM_NOT_FOUND' WHERE lead_table = 'working_sheet' AND lead_id = ?`, [workingSheetId]);
            return;
        }

        const [claimRes] = await db.query(`
            UPDATE whatsapp_conversations 
            SET rm_session_message_status = 'PENDING' 
            WHERE lead_table = 'working_sheet' AND lead_id = ? 
            AND rm_session_message_status IN ('NOT_REQUIRED', 'RM_NOT_FOUND', 'FAILED')
        `, [workingSheetId]);

        if (claimRes.affectedRows === 0) return;

        const customerName = convo.customer_name || 'Customer';
        const messageText = await renderServiceMessage('BOT_RM_ASSIGNED', {
            customer_name: customerName,
            rm_name: rmName,
            rm_mobile: rmMobile
        });

        const sendResult = await sendSessionMessage({
            phoneNumber: convo.normalized_number,
            messageText: messageText
        });

        if (sendResult.success) {
            await db.query(`
                UPDATE whatsapp_conversations 
                SET rm_session_message_status = 'SENT',
                    rm_session_message_id = ?,
                    rm_session_message_sent_at = NOW(),
                    last_outbound_message = ?,
                    last_activity_at = NOW()
                WHERE lead_table = 'working_sheet' AND lead_id = ?
            `, [sendResult.messageId, messageText, workingSheetId]);
            console.log(`[BotQueue] Sent BOT RM Assignment message to working sheet ID: ${workingSheetId}`);

            // Phase 2: Automation Trigger
            queueAutomations('BOT_RM_ASSIGNED', 'BOT', 'working_sheet', workingSheetId, telecallerId, convo.normalized_number, convo.normalized_number, {
                customer_name: customerName,
                rm_name: rmName,
                rm_mobile: rmMobile
            });
        } else {
            await db.query(`
                UPDATE whatsapp_conversations 
                SET rm_session_message_status = 'FAILED'
                WHERE lead_table = 'working_sheet' AND lead_id = ?
            `, [workingSheetId]);
            console.log(`[BotQueue] Failed to send BOT RM Assignment message to working sheet ID: ${workingSheetId}`);
        }

    } catch (error) {
        console.error("[BotQueue] Error sending RM assignment message:", error);
    }
};

const ensureQueueTable = async () => {
  try {
    await db.query(`
      CREATE TABLE IF NOT EXISTS bot_lead_fetch_queue (
        id INT AUTO_INCREMENT PRIMARY KEY,
        telecaller_id INT NOT NULL,
        status ENUM('WAITING','ASSIGNED','CANCELLED') DEFAULT 'WAITING',
        queued_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        assigned_at DATETIME NULL,
        assigned_working_sheet_id INT NULL,
        last_seen_at DATETIME NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        UNIQUE KEY uniq_bot_queue_telecaller (telecaller_id),
        INDEX idx_bot_queue_status_time (status, queued_at)
      )
    `);
  } catch (err) {
    console.error("Failed to ensure bot_lead_fetch_queue table:", err);
  }
};

// Ensure table on load
ensureQueueTable();

const checkEligibility = async (telecallerId) => {
  try {
    // Check if BOT leads are paused for this telecaller
    const [tmRows] = await db.query(
      `SELECT bot_leads_paused FROM telecaller_master WHERE id = ?`,
      [telecallerId]
    );

    if (tmRows.length > 0 && tmRows[0].bot_leads_paused) {
      return {
        eligible: false,
        blockingLead: null,
        message: "BOT Lead fetching is currently paused by Admin.",
        isPaused: true
      };
    }

    // Check latest assigned bot lead for this telecaller
    const [rows] = await db.query(
      `SELECT * FROM working_sheet 
       WHERE telecaller_id = ? AND source = 'BOT_POOL' AND (is_kyc_done = 0 OR is_kyc_done IS NULL)
       AND (is_closed_lead = 0 OR is_closed_lead IS NULL) 
       AND (is_transferred_lead = 0 OR is_transferred_lead IS NULL)
       AND (is_released_to_free_pool = 0 OR is_released_to_free_pool IS NULL)
       ORDER BY id DESC LIMIT 1`,
      [telecallerId]
    );

    if (rows.length === 0) {
      return { eligible: true, blockingLead: null };
    }

    const latestLead = rows[0];
    if (!latestLead.status1 || String(latestLead.status1).trim() === '' || String(latestLead.status1).trim() === 'None') {
      return {
        eligible: false,
        blockingLead: latestLead,
        message: "Please update Status 1 for your current lead before fetching a new lead."
      };
    }

    return { eligible: true, blockingLead: null };
  } catch (err) {
    console.error("Error checking eligibility:", err);
    throw err;
  }
};

const assignLeadToTelecaller = async (telecallerId, manualQueueId = null) => {
  const connection = await db.getConnection();
  try {
    await connection.beginTransaction();

    // 1. Check if new_leads has available leads using LIFO (ORDER BY id DESC LIMIT 1)
    const [leads] = await connection.query(
      `SELECT * FROM new_leads ORDER BY id DESC LIMIT 1 FOR UPDATE`
    );

    if (leads.length === 0) {
      await connection.rollback();
      return { assigned: false, lead: null };
    }

    const lead = leads[0];

    // 2. Insert selected lead into working_sheet
    // Omit contact_last10 as it is STORED GENERATED
    const [insertRes] = await connection.query(
      `INSERT INTO working_sheet (lead_name, lead_contact, telecaller_id, source, is_active, created_at)
       VALUES (?, ?, ?, 'BOT_POOL', 1, NOW())`,
      [lead.lead_name, lead.lead_contact, telecallerId]
    );

    const newWorkingSheetId = insertRes.insertId;

    // 3. Delete from new_leads
    await connection.query(`DELETE FROM new_leads WHERE id = ?`, [lead.id]);

    // 4. Update queue if they were in it (mark ASSIGNED so they can re-queue later if needed)
    await connection.query(
      `UPDATE bot_lead_fetch_queue 
       SET status = 'ASSIGNED', assigned_at = NOW(), assigned_working_sheet_id = ?
       WHERE telecaller_id = ?`,
      [newWorkingSheetId, telecallerId]
    );

    // 5. Migrate conversation
    await connection.query(`
       UPDATE whatsapp_conversations
       SET
         lead_table = 'working_sheet',
         lead_id = ?,
         telecaller_id = ?,
         updated_at = NOW()
       WHERE
         lead_type = 'BOT'
         AND lead_table = 'new_leads'
         AND lead_id = ?
    `, [newWorkingSheetId, telecallerId, lead.id]);

    // Migrate whatsapp_message_queue to maintain webhook linkage
    await connection.query(`
       UPDATE whatsapp_message_queue
       SET
         lead_table = 'working_sheet',
         lead_id = ?
       WHERE
         lead_type = 'BOT'
         AND lead_table = 'new_leads'
         AND lead_id = ?
    `, [newWorkingSheetId, lead.id]);

    // Insert history
    await connection.query(`
        INSERT INTO bot_lead_assignment_history 
        (original_new_lead_id, working_sheet_id, telecaller_id, assignment_mode, manual_queue_id)
        VALUES (?, ?, ?, 'MANUAL_FETCH', ?)
    `, [lead.id, newWorkingSheetId, telecallerId, manualQueueId]);

    await connection.commit();

    // 6. Asynchronously send RM assignment message (if applicable)
    sendBotAssignmentMessage(newWorkingSheetId, telecallerId).catch(console.error);

    // 7. Asynchronously trigger Interakt Chat Assignment (new API without wc_id)
    assignChatToAgent(lead.lead_contact, telecallerId).catch(console.error);

    // 8. Asynchronously sync Lead Traits (Name & RM) to Interakt
    syncLeadTraits(lead.lead_contact, lead.lead_name, telecallerId).catch(console.error);

    // Fetch the newly inserted lead to return
    const [newLeads] = await db.query(`SELECT * FROM working_sheet WHERE id = ?`, [newWorkingSheetId]);
    return { assigned: true, lead: newLeads[0] };
  } catch (err) {
    await connection.rollback();
    console.error("Error assigning lead in transaction:", err);
    throw err;
  } finally {
    connection.release();
  }
};

const processBotLeadQueue = async () => {
  try {
    console.log("[BotQueue] Starting queue processor...");
    
    while (true) {
      // Check if new_leads has any rows
      const [leadCountRes] = await db.query("SELECT COUNT(*) as cnt FROM new_leads");
      if (leadCountRes[0].cnt === 0) {
        break; // No more leads to assign
      }

      // Check waiting queue FIFO (ORDER BY queued_at ASC)
      const [waitingQueue] = await db.query(
        `SELECT id, telecaller_id FROM bot_lead_fetch_queue 
         WHERE status = 'WAITING' 
         ORDER BY queued_at ASC LIMIT 10`
      );

      if (waitingQueue.length === 0) {
        // No waiting telecallers. Try Auto Assign.
        if (!autoAssignService) {
            autoAssignService = require('./botAutoAssignService');
        }
        
        const autoRes = await autoAssignService.assignAutoTop10Lead();
        if (autoRes.result === 'ASSIGNED') {
            console.log(`[BotQueue] Auto-assigned lead to working_sheet ${autoRes.workingSheetId} for Telecaller ${autoRes.telecallerId}`);
            sendBotAssignmentMessage(autoRes.workingSheetId, autoRes.telecallerId).catch(console.error);
            continue; // Keep draining
        } else if (autoRes.result === 'MANUAL_QUEUE_APPEARED') {
            continue; // Re-evaluate queue
        } else {
            // DISABLED, NO_LEADS, or NO_ELIGIBLE_TELECALLER
            break; 
        }
      }

      let assignedInThisLoop = false;

      for (const queueItem of waitingQueue) {
        // Check eligibility before assigning
        const eligibility = await checkEligibility(queueItem.telecaller_id);
        if (!eligibility.eligible) {
          console.log(`[BotQueue] Telecaller ${queueItem.telecaller_id} is waiting but ineligible (pending Status 1). Skipping.`);
          continue;
        }

        // Try assigning
        const res = await assignLeadToTelecaller(queueItem.telecaller_id, queueItem.id);
        if (res.assigned) {
          console.log(`[BotQueue] Automatically assigned lead ID ${res.lead.id} to Telecaller ${queueItem.telecaller_id}`);
          assignedInThisLoop = true;
          break; // Break the for loop and re-check queue from start (FIFO)
        } else {
          // No more leads
          break;
        }
      }

      if (!assignedInThisLoop) {
        // No eligible telecallers were able to receive a lead or no leads available
        break;
      }
    }
    
    console.log("[BotQueue] Queue processing completed.");
  } catch (err) {
    console.error("[BotQueue] Error processing queue:", err);
  }
};

module.exports = {
  ensureQueueTable,
  checkEligibility,
  assignLeadToTelecaller,
  processBotLeadQueue,
  sendBotAssignmentMessage // exported to be used by auto assign service if needed
};
