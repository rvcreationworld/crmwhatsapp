...
exports.toggleAutoSync = async (req, res) => {
  try {
    ...
    await db.query("UPDATE telecaller_campaigns SET auto_sync = ? WHERE id = ?", [auto_sync, id]);
    res.json({ message: "Auto-sync setting updated" });
  } catch (error) {
    console.error("Toggle auto sync error:", error);
    res.status(500).json({ message: "Server error updating auto-sync" });
  }
};
