  ...
  try {
    ...
    if (type === 'bot') {
      query = `
        ...
        FROM working_sheet
        WHERE telecaller_id = ? AND source IN ('BOT_POOL', 'BOT_AUTO_ASSIGN') AND (is_released_to_free_pool = 0 OR is_released_to_free_pool IS NULL) AND (is_closed_lead = 0 OR is_closed_lead IS NULL) AND (is_transferred_lead = 0 OR is_transferred_lead IS NULL)
          AND (is_kyc_done = 0 OR is_kyc_done IS NULL)
          AND (status_lock_type IS NULL OR status_lock_type != 'KYC_DONE')
          AND MONTH(created_at) = MONTH(CURRENT_DATE())
          AND YEAR(created_at) = YEAR(CURRENT_DATE())
          AND (status1 IS NULL OR status1 = '' OR LOWER(status1) IN ('none', 'new'))
          AND (status2 IS NULL OR status2 = '' OR LOWER(status2) IN ('none', 'new'))
          AND (status3 IS NULL OR status3 = '' OR LOWER(status3) IN ('none', 'new'))
          /* AND NOT EXISTS (
            SELECT 1 
            FROM callpulse_call_logs ccl
            WHERE ccl.telecaller_id = working_sheet.telecaller_id
              AND (
                (ccl.lead_type = 'BOT' AND ccl.lead_id = working_sheet.id)
                OR ccl.normalized_number = RIGHT(REGEXP_REPLACE(working_sheet.lead_contact, '[^0-9]', ''), 10)
              )
          ) */
