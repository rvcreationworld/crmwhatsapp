...
async function run() {
  ...
  try {
    const query = `
      ...
      LEFT JOIN whatsapp_conversations wc ON working_sheet.lead_contact COLLATE utf8mb4_unicode_ci = wc.phone_number COLLATE utf8mb4_unicode_ci
      LIMIT 1
    `;
    const [rows] = await conn.execute(query);
    console.log("Success with COLLATE");
  } catch(e) {
