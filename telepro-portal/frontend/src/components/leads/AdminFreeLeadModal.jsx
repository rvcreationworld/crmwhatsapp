...
import { Loader2, X, Phone, User, Clock, Building2, UserCircle2 } from "lucide-react";
...
const AdminFreeLeadModal = ({ isOpen, onClose, leadId }) => {
  const [activeTab, setActiveTab] = useState('PROFILE');

  const { data: detailsData, isLoading, isError } = useQuery({
    queryKey: ["adminFreeLeadDetails", leadId],
    queryFn: async () => {
      const res = await api.get(`/api/admin/free-leads/${leadId}`);
      return res.data;
    },
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-slate-50 dark:bg-[#151521] rounded-xl p-5 border border-slate-200 dark:border-slate-800">
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    <div className="flex justify-between">
                      <span className="text-slate-500">Free Status:</span>
                      <span className="font-semibold text-slate-900 dark:text-white">{lead?.free_status}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-slate-500">Moved/Fetched At:</span>
                      <span className="font-semibold text-slate-900 dark:text-white">
                        {new Date(lead?.fetched_at || lead?.moved_to_free_at || lead?.created_at).toLocaleString()}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-slate-50 dark:bg-[#151521] rounded-xl p-5 border border-slate-200 dark:border-slate-800">
                  <h3 className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-4 flex items-center gap-2">
                    <UserCircle2 size={16} /> Telecaller Assignment
                  </h3>
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      <span className="text-slate-500">Previous Telecaller:</span>
...
export default AdminFreeLeadModal;
