...
exports.getMyLeadNumbers = async (req, res) => {
  try {
    ...
    const allLeads = [...freeLeads, ...directLeads, ...botLeads];
...
exports.uploadLogs = async (req, res) => {
  try {
      ...
      try {
        await db.query(
          `INSERT INTO callpulse_call_logs 
           (telecaller_id, lead_type, lead_id, dialed_number, normalized_number, call_type, call_started_at, call_ended_at, duration_seconds, sync_status, synced_at) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'SYNCED', NOW())
           ...
           synced_at = NOW()`,
          [telecallerId, lead_type, dbLeadId, dialed_number, normalized_number, call_type, call_started_at, call_ended_at, duration_seconds]
        );
        insertedCount++;
        console.log(`SQL insert/update result: Success for ${lead_type} ${dbLeadId}`);
      } catch (err) {
        failedCount++;
        failedItems.push({
          lead_type, lead_id, normalized_number,
    ...
    if (logs.length === 1) {
      if (duplicateCount === 1) {
        return res.json({ success: true, inserted: false, duplicate: true });
      } else if (insertedCount === 1) {
        return res.json({ success: true, inserted: true });
      } else if (skippedCount === 1) {
        return res.json({ success: true, inserted: false, skipped: true, reason: "Unmatched personal call" });
      } else {
        return res.json({ success: false, inserted: false, reason: failedItems[0]?.reason });
      }
    }
