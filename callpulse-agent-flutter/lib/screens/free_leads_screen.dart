...
import '../services/free_leads_service.dart';
...
class FreeLeadsScreen extends StatefulWidget {
  const FreeLeadsScreen({super.key});

  @override
  State<FreeLeadsScreen> createState() => _FreeLeadsScreenState();
}

class _FreeLeadsScreenState extends State<FreeLeadsScreen> {
  final FreeLeadsService _freeLeadsService = FreeLeadsService();
  ...
  String? _selectedStatus4;

  final List<String> _status4Options = [
  ...
  Future<void> _loadStatus() async {
    ...
    final res = await _freeLeadsService.getStatus();
    if (!mounted) return;
    ...
    if (res['success'] == true) {
      setState(() {
        ...
        if (_poolStatus?['assignedLead'] != null) {
          final currentSt1 = _poolStatus!['assignedLead']['status4'];
          _selectedStatus4 = (currentSt1 != null && _status4Options.contains(currentSt1)) ? currentSt1 : null;
      ...
      setState(() {
        _isLoading = false;
        _errorMessage = res['message'] ?? 'Failed to load Free Leads status';
      });
    }
  }
  ...
  Future<void> _handleFetchLead() async {
    ...
    final res = await _freeLeadsService.fetchLead();
    if (!mounted) return;
  ...
  Future<void> _handleUpdateStatus4() async {
    final assigned = _poolStatus?['assignedLead'];
    if (assigned == null) return;

    if (_selectedStatus4 == null) {
    ...
    final res = await _freeLeadsService.updateStatus4(assigned['id'], _selectedStatus4 ?? '', _remarkController.text.trim());
    if (!mounted) return;
    setState(() => _isSaving = false);

    if (res['success'] == true) {
      PremiumSnackbar.show(context, 'Status 4 updated! Lead moved to Current Leads.', isSuccess: true);
      _remarkController.clear();
      _selectedStatus4 = null;
      await _loadStatus();
    } else {
      setState(() => _errorMessage = res['message'] ?? 'Failed to update Status 4');
    }
  }
  ...
  Future<void> _makePhoneCall(String phoneNumber) async {
    ...
    try {
      final assigned = _poolStatus?['assignedLead'];
      if (assigned != null && assigned['id'] != null) {
        final prefs = await SharedPreferences.getInstance();
        await prefs.setString('activeLeadType', 'FREE');
        await prefs.setInt('activeLeadId', assigned['id'] is String ? int.parse(assigned['id']) : assigned['id']);
        
        String norm = cleaned;
        if (norm.length > 10) norm = norm.substring(norm.length - 10);
        await prefs.setString('activeNumber', norm);
      }
    } catch (e) {
      print("Error saving call context: $e");
    }
  ...
  Widget build(BuildContext context) {
    ...
    final bool isKycDone = assignedLead?['is_kyc_done'] == 1 || assignedLead?['is_kyc_done'] == true || assignedLead?['status4'] == 'KYC Done';
    final bool isUnderUs = assignedLead?['status_lock_type'] == 'UNDER_US' || assignedLead?['status4'] == 'Under Us';
    final bool isMidnightLocked = assignedLead?['is_locked'] == true || assignedLead?['status4_locked'] == true;
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      const Text('Free Leads Available', style: TextStyle(color: AppColors.textMuted, fontSize: 12, fontWeight: FontWeight.w500, letterSpacing: 0.5)),
            ...
            GlassCard(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const Text('Update Status 4', style: TextStyle(color: AppColors.textPrimary, fontSize: 15, fontWeight: FontWeight.w700)),
                  const SizedBox(height: 14),

                  GlassDropdown(
                    value: _selectedStatus4,
                    hint: 'Select Status',
                    items: _status4Options,
                    label: 'Status 4',
                    enabled: !isLocked,
                    onChanged: (val) { if (val != null) setState(() => _selectedStatus4 = val); },
                  ),
                  const SizedBox(height: 14),
                  ...
                  NeonButton(
                    ...
                    onPressed: isLocked ? null : (_isSaving ? null : _handleUpdateStatus4),
                    isLoading: _isSaving,
                  ),
                ],
              ),
