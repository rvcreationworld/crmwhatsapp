...
import '../services/transferred_lead_service.dart';
...
class TransferredLeadsScreen extends StatefulWidget {
  const TransferredLeadsScreen({super.key});

  @override
  State<TransferredLeadsScreen> createState() => _TransferredLeadsScreenState();
}
...
class _TransferredLeadsScreenState extends State<TransferredLeadsScreen> {
  ...
  Future<void> _loadTransferredLeads() async {
    ...
    if (res['success'] == true) {
      ...
      setState(() {
        _isLoading = false;
        _errorMessage = res['message'] ?? 'Failed to load Transferred Leads';
      });
    }
  }

  Future<void> _handleUpdateStatus1(int leadId) async {
    final status4 = _selectedStatusMap[leadId];
    if (status4 == null) {
    ...
    if (res['success'] == true) {
      ...
      PremiumSnackbar.show(context, res['message'] ?? 'Failed to update Status 4', isError: true);
                    ...
                    child: GlassCard(
                      ...
                      child: Row(children: [
                        ...
                        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          const Text('Fresh Transferred Leads', style: TextStyle(color: AppColors.textMuted, fontSize: 12, letterSpacing: 0.5)),
