const fs = require('fs');
let file = fs.readFileSync('backend/controllers/telecallerBotPoolController.js', 'utf8');
...
const targetQuery = `SELECT * FROM working_sheet 
       ...
       AND (is_closed_lead = 0 OR is_closed_lead IS NULL) AND (is_transferred_lead = 0 OR is_transferred_lead IS NULL)
...
fs.writeFileSync('backend/controllers/telecallerBotPoolController.js', file);
console.log('Fixed bot pool query');
